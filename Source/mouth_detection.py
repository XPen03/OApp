...
# added a new input to save the images with the name correctly
# cancelled to display the pictures on the screen
def mouth_location(image_path, image_name, predictor_path):
    ...
    if not is_only_one_person(rect):
        pass # here it was a quit before
    else:
        ...
        save_name = 'mouth_' + image_name
        image_path = os.path.join(save_path,save_name)
        cv2.imwrite(image_path, mouth)
        #cv2.waitKey(0)   
