...
def compare_result(expect_result,result):
    return result in expect_result

def compare_results(expect_results, results):
    count = 0
    for i in range(len(results)):
        if results[i] in expect_results[i]:
            count += 1
    return print("Result : %d error out of %d. Accuracy: %.2f" %(len(results) - count, len(results), count/len(results)))
